...
func load_save_data(data: Dictionary, parent_node: Node2D) -> void:
	"""Load layout data from save"""
	clear_all()
	
	if not data.has("pieces"):
		return
	
	for piece_data: Dictionary in data["pieces"]:
		var piece_id: String = piece_data["piece_id"]
		var grid_pos: Vector2i = Vector2i(piece_data["grid_pos"]["x"], piece_data["grid_pos"]["y"])
		var rotation: int = piece_data["rotation"]
		
		# Instantiate the piece
		var registry_data: BuildingPieceRegistry.PieceData = BuildingPieceRegistry.get_piece(piece_id)
		if not registry_data:
			push_warning("LayoutData: Unknown piece in save data: %s" % piece_id)
			continue
		
		var scene: PackedScene = load(registry_data.scene_path)
		if not scene:
			push_warning("LayoutData: Failed to load scene: %s" % registry_data.scene_path)
			continue
		...
		place_piece(piece_id, grid_pos, rotation, instance)
