...
func _try_place_selected_item() -> void:
	...
	var slot_data: Dictionary = InventoryManager.player_hotbar.get_slot(hotbar_slot_index)
	...
	if new_quantity > 0:
		InventoryManager.player_hotbar.set_slot(hotbar_slot_index, selected_item.id, new_quantity)
	else:
		InventoryManager.player_hotbar.clear_slot(hotbar_slot_index)
